...
def backup_folder(selected_map, selected_save):
    ...
    try:
        ...
        messagebox.showinfo("Başarılı", f"Yedekleme başarılı: {
                            destination_path}")
    except Exception as e:
        messagebox.showerror(
            "Hata", f"Yedekleme sırasında bir hata oluştu: {str(e)}")
...
def delete_folder(selected_map, selected_save):
    ...
    user_response = messagebox.askyesno(
        "Onay", f"'{selected_save}' Dosyasını silmek istediğinize emin misiniz?")
    if user_response:
        try:
            ...
            messagebox.showerror(
                "Hata", f"Silme sırasında bir hata oluştu: {str(e)}")
    else:
...
# Dosya silme işlemini yapan fonsiyon
...
for map_name in os.listdir(appdata_path):
    if(os.path.isdir(appdata_path+ "\\" +map_name)):
